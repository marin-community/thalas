...
from thalas.execution.executor import Executor, _step_dag_tpu_regions, resolve_executor_step
from thalas.execution.remote import RemoteCallable, remote
from thalas.execution.step_runner import StepRunner
from thalas.execution.step_spec import StepSpec
from thalas.execution.types import ExecutorStep
...
def iris_active():
    ...
    with patch("thalas.execution.executor._iris_backend_is_active", return_value=True):
        yield
...
def test_resolve_executor_step_raises_for_dual_region_bucket_location(iris_active, remote_step):
    with patch("thalas.execution.executor.get_bucket_location", return_value="NAM4"):
...
def test_resolve_executor_step_skips_bucket_location_permission_failures(iris_active, remote_step):
    ...
    with patch(
        "thalas.execution.executor.get_bucket_location",
...
def test_step_dag_tpu_regions_uses_iris_variant_regions_when_not_pinned():
    ...
    with patch(
        "thalas.execution.executor._regions_for_tpu_variant_from_iris",
